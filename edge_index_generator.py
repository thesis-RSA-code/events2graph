...
def main(args):
    ...
    # Create node selection filter if specified
    node_selection = None
    if args.node_selection is not None:
        node_selection = NodeSelection(
            time_cut=args.node_selection.get('time_cut'),
            charge_cut=args.node_selection.get('charge_cut')
        )
        print(f"Node selection filter created with:")
        if args.node_selection.get('time_cut'):
            print(f"  Time cut: {args.node_selection['time_cut']}")
        if args.node_selection.get('charge_cut'):
            print(f"  Charge cut: {args.node_selection['charge_cut']}")
    ...
    with h5py.File(args.h5_data_file, 'r', libver='latest', swmr=False) as data_file, \
        ...
        for event_idx, event_name in enumerate(tqdm(event_names, desc="Generating edge indices")):
            ...
            # Check if this edge dataset already exists
            ...
            for feature in args.pos_features:
                if feature not in data_event_group:
                    print(f"Warning: Feature '{feature}' not found in {event_name}. Event skipped.")
                    break
                raw_edge_coords.append(data_event_group[feature][:])
            else:
                # All features found, stack them
                raw_edge_coords = np.stack(raw_edge_coords, axis=1)
                if raw_edge_coords.shape[0] == 0:
                    continue

                # Apply node selection filtering if specified
                if node_selection is not None:
                    if 'time' not in data_event_group:
                        print(f"Warning: 'time' not found in {event_name}. Node selection requires time data. Event skipped.")
                        continue
                    if 'charge' not in data_event_group:
                        print(f"Warning: 'charge' not found in {event_name}. Node selection requires charge data. Event skipped.")
                        continue
                    
                    time_data = data_event_group['time'][:]
                    charge_data = data_event_group['charge'][:]
                    
                    # Apply filtering
                    filtered_coords = node_selection.apply_selection(raw_edge_coords, time_data, charge_data)
                ...
                # Save edge index directly as event dataset (no groups)
